...
fn get_http_via_tcp(
    ...
    let mut stream = TcpStream::connect((host.as_str(), port)).map_err(Error::Io)?;
...
fn parse_http_response_from_openssl_output(
    ...
    let status_line = lines.next().ok_or(Error::InvalidUrl)?;
    let mut status_parts = status_line.split_whitespace();
    let _httpver = status_parts.next().ok_or(Error::InvalidUrl)?;
    let code_str = status_parts.next().ok_or(Error::InvalidUrl)?;
    let code: u16 = code_str
        .parse()
        .map_err(|_| Error::BadStatusCode(code_str.into()))?;
...
fn decode_chunked(mut body: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    loop {
        ...
        let size = usize::from_str_radix(size_hex, 16).map_err(|_| Error::InvalidUrl)?;
